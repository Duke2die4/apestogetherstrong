...
public class Sphouse3Structure extends ApesStrongTogetherModElements.ModElement {
	private static Feature<NoFeatureConfig> feature = null;
	private static ConfiguredFeature<?, ?> configuredFeature = null;
	public Sphouse3Structure(ApesStrongTogetherModElements instance) {
		super(instance, 188);
	...
	private static class FeatureRegisterHandler {
		...
		public void registerFeature(RegistryEvent.Register<Feature<?>> event) {
				...
				public boolean generate(ISeedReader world, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
					...
					if ((random.nextInt(1000000) + 1) <= 10000) {
						...
						for (int a = 0; a < count; a++) {
							...
							BlockPos spawnTo = new BlockPos(i + -12, j + -12, k + -10);
							int x = spawnTo.getX();
							int y = spawnTo.getY();
							int z = spawnTo.getZ();
							Template template = world.getWorld().getStructureTemplateManager()
									.getTemplateDefaulted(new ResourceLocation("apes_strong_together", "sp_house3"));
							if (template == null)
								return false;
			...
			event.getRegistry().register(feature.setRegistryName("sphouse_3"));
			Registry.register(WorldGenRegistries.CONFIGURED_FEATURE, new ResourceLocation("apes_strong_together:sphouse_3"), configuredFeature);
		}
	}
	@SubscribeEvent
